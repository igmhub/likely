...
namespace likely {
	...
	public:
        ...
        double integrateAll();
        // Returns the integral of integrand(x)*osc(omega*x) from [a,b] where
        // osc = sin or cos. Updates the value return by getAbsError(). Uses GSL QAWO.
        double integrateOsc(double a, double b, double omega, bool useSin = true);
        // Returns the integral of integrand(x)*osc(omega*x) from [a,+infinity) where
        // osc = sin or cos. Updates the value return by getAbsError(). Uses GSL QAWF.
        double integrateOscUp(double a, double omega, bool useSin = true);
